...
macro_rules! gen_shorthands {
    ($(($s:ident,$i:literal,$off:literal,$cl:ident)),* ) => {
        impl ControlSignals {
            const fn off_mask() -> u64 {
                let mut bitfield = 0;
                $(bitfield = bit_set(bitfield,$i,$off));*;
                bitfield
            }
...
gen_shorthands! {
    ...
    (PC_PLUS,8,0,InvClock),    (LD_PC,9,1,Now),      (OUT_PC,10,1,Now),    (STC_PLUS,11,0,Now),  (STC_TICK,12,0,InvClock),      (LD_STC,13,1,Now),        (OUT_STC,14,1,Now),   (LD_MAR,15,0,Clock),    //B
    ...
    (MC_PLUS,32,0,InvClock),   (OUT_FLAG,33,1,Now),  (USE_SWITCH,34,1,Now)                                                                                                   //E
}
...
impl ControlSignals {
    ...
    pub const OFF : u64 = Self::off_mask();

    pub fn init() -> Self {
        Self { bitfield: Self::OFF }
    }
    ...
    pub fn get(&self, index: usize) -> u64 {
        assert!(index < 64);
        let v = (self.bitfield >> (63 - index)) & 1;
        //println!("Asked for {index}, got {v}");
        v
    }
    ...
    pub fn filter(&self, mask: u64) -> Self {
        Self {
            bitfield: (self.bitfield & mask) | (Self::OFF & !mask),
        }
    }
}
